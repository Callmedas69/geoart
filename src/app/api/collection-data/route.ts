...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Use the all metadata endpoint for collection data
    const response = await fetch(
      `${VIBEMARKET_BASE_URL}/contractAddress/${contractAddress}/all?${queryParams}`,
      {
        headers,
        cache: 'force-cache',
      }
    ...
    if (!response.ok) {
      console.warn(`VibeMarket collection API returned ${response.status}`)
      return NextResponse.json(
    ...
    console.error('Collection data API error:', error)
    return NextResponse.json(
